...
### 📚 Ý nghĩa các chỉ số Metric trong benchmark

**Answer-side metrics (chấm chất lượng câu trả lời của agent):**

| Metric | Ý nghĩa | Công thức / Cách đo | Range | Tốt | Xấu |
|--------|---------|-------------------|:-----:|:---:|:---:|
| **Faithfulness** | Answer có grounded trong context không? Có bịa thông tin không? | Word-overlap: `\|answer ∩ context\| / \|answer\|` (heuristic) hoặc LLM NLI check (LLM-based) | 0–1 | ≥ 0.7 | < 0.3 = hallucination |
| **Relevance / Answer Relevancy** | Answer có trả lời đúng câu hỏi không? | Word-overlap: `\|answer ∩ question\| / \|question\|` hoặc LLM semantic judge | 0–1 | ≥ 0.7 | < 0.3 = irrelevant |
| **Completeness** | Answer có cover đủ expected answer không? | Word-overlap: `\|answer ∩ expected\| / \|expected\|` hoặc LLM judge so sánh | 0–1 | ≥ 0.7 | < 0.3 = incomplete |

**Retrieval-side metrics (chấm chất lượng retrieval — áp dụng trên danh sách chunks):**

| Metric | Ý nghĩa | Công thức | Range | Tốt | Xấu |
|--------|---------|-----------|:-----:|:---:|:---:|
| **Context Recall** | Retriever có lấy đủ evidence cần thiết không? | `\|expected_tokens ∩ union(all chunks)\| / \|expected_tokens\|` | 0–1 | ≥ 0.7 | < 0.5 = missing evidence |
| **Context Precision** | Chunks relevant có được xếp lên đầu không? Đo rank-aware | AP@K: Average Precision qua các vị trí, thưởng relevant ở top | 0–1 | ≥ 0.7 | < 0.5 = ranking kém |

**Failure types và ý nghĩa:**
- **hallucination** (faithfulness < 0.3): Answer bịa thông tin — nguy hiểm nhất, có thể gây misinformation
- **irrelevant** (relevance < 0.3): Answer không giải quyết câu hỏi — prompt ambiguous hoặc intent detection sai
- **incomplete** (completeness < 0.3): Answer bỏ sót key information — context window nhỏ hoặc retrieval thiếu
- **off_topic**: Answer lạc chủ đề — intent detection sai
- **refusal**: Từ chối sai context — guardrails quá chặt

> **Lưu ý quan trọng:** Heuristic word-overlap chỉ đo lexical similarity (đếm từ khớp nhau), không hiểu ngữ nghĩa. LLM-based evaluation (Gemini) hiểu semantic nên cho kết quả chính xác hơn. Sự khác biệt này thể hiện rõ qua scores: heuristic cho relevance 0.72 nhưng Gemini chỉ cho 0.025–0.038 vì mock agent dù có từ khóa giống câu hỏi nhưng không thực sự trả lời đúng.

Đã chạy thực tế **4 frameworks** trên cùng 20 QA pairs với `GOOGLE_API_KEY` (Gemini 2.5 Flash) — sử dụng `benchmark_v2.py`:

| Tiêu chí | Framework 1: RAGAS Heuristic | Framework 2: RAGAS-style (Gemini) | Framework 3: TruLens-style (Gemini) | Framework 4: LLM-as-Judge (Gemini 2.5 Flash) |
|----------|:---------------------------:|:---------------------------------:|:-----------------------------------:|:-------------------------------------------:|
| Setup complexity | Low — word overlap functions | Medium — Gemini via google-genai SDK | Medium — Gemini via google-genai SDK | Medium — Gemini via google-genai SDK |
| Metrics available | Faithfulness, Relevance, Completeness, Context Recall, Context Precision | Faithfulness, Answer Relevancy, Context Recall, Context Precision | Faithfulness, Relevance, Completeness | Faithfulness, Relevance, Completeness |
| CI/CD integration | Custom script + threshold check | Custom script + Gemini API | Custom script + Gemini API | Custom script + Gemini API |
...
| Insight rút ra | Heuristic nhanh nhưng over-estimate relevance và under-estimate hallucination | Gemini đánh giá semantic → relevance/faithfulness thấp hơn heuristic; Context Recall/Precision ~70% cho thấy dataset quality tốt | Completeness gần 0 → mock agent không cover expected answer dù có context | Combined prompt cho scores thấp nhất — strict nhất trong 4 frameworks |

**Câu hỏi phân tích (với số liệu thực tế):**
- **Scores có consistent giữa các frameworks không?** Không. RAGAS heuristic cho relevance cao (0.72) vì mock agent chứa từ từ question. Cả 3 LLM-based frameworks (RAGAS-style 0.030, TruLens-style 0.038, LLM-as-Judge 0.025) đều cho relevance gần 0 — Gemini đánh giá answer không trả lời đúng câu hỏi dù có chứa cùng từ khóa. Đây là minh chứng rõ ràng cho sự khác biệt giữa **word overlap** và **semantic understanding**.
- **Framework nào strict hơn? Tại sao?** **LLM-as-Judge (Gemini 2.5 Flash)** strict nhất: faithfulness=0.025 vs heuristic=0.075. Gemini hiểu rằng câu trả lời generic "Based on my knowledge: ... involves key AI concepts" không grounded trong context. Heuristic chỉ check word overlap nên vẫn cho điểm dương.
- **Context Recall/Precision có ý nghĩa gì?** RAGAS-style cho Context Recall=0.703 và Context Precision=0.740 — chứng tỏ golden dataset có chất lượng tốt và retrieval coverage đầy đủ. Vấn đề là generation step (mock agent không sử dụng context).
- **Failure cases có giống nhau không?** Có — cả 4 framework đều detect hallucination. Nhưng mức độ strict khác nhau: LLM-as-Judge cho ~0.025, RAGAS-style và TruLens-style cho ~0.100, heuristic cho ~0.075.

**Kết luận từ real benchmark:**
> Word-overlap heuristic (dùng trong lab) **over-estimate relevance** (0.72 vs 0.03-0.04 thực tế) và **under-estimate hallucination** so với LLM judge thực tế. Trong production, nên dùng **LLM-based evaluation** (Gemini) để có semantic understanding. Pipeline khuyến nghị: **Staging dùng heuristic** (5 giây, fast regression check) → **Production gate dùng Gemini LLM judge** (5 phút, accurate) — kết hợp cả tốc độ lẫn độ chính xác.
