...
|----------|-------------------|-------------------|
| Setup complexity | Low — chỉ cần word overlap functions | Medium — cần API key cho LLM-based metrics |
| Metrics available | Faithfulness, Relevance, Completeness, Context Recall, Context Precision | Faithfulness, Hallucination, Answer Relevancy, Contextual Recall/Precision, Toxicity, Bias |
| CI/CD integration | Custom script + threshold check | Native pytest integration: `deepeval test run` |
| Score cho cùng dataset | Faithfulness avg: 0.09, Relevancy avg: 0.83 | (Would need actual LLM judge) |
| Insight rút ra | Heuristic nhanh nhưng không capture semantic nuance | LLM-based accurate hơn nhưng cost cao hơn |

**Câu hỏi phân tích:**
- **Scores có consistent giữa 2 frameworks không?** Không hoàn toàn. RAGAS heuristic dùng word overlap nên dễ under-estimate faithfulness khi answer dùng synonym. DeepEval dùng LLM judge nên cho score thực tế hơn nhưng có bias.
- **Framework nào strict hơn? Tại sao?** Heuristic strict hơn với faithfulness (vì overlap thấp = score thấp), trong khi LLM judge có thể lenient nếu semantic đúng dù từ khác.
- **Failure cases có giống nhau không?** Có — cả 2 đều detect hallucination khi answer không grounded trong context. Nhưng threshold khác nhau.
